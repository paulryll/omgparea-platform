...
router.get('/students/:id', async (req, res) => {
  ...
  const gateByKey = new Map(gateQ.rows.map((g) => [`${g.module_id}:${g.section_id ?? 'null'}`, g]));
          ...
          return {
            ...
            gate: g && {
              ...
              unlockedBy: g.unlocked_by
                ? `${g.unlocker_first} ${g.unlocker_last}` : null,
              note: g.note,
            },
          };
        });
...
// Unlocks a single gate for a specific student. Idempotent.
...
router.post('/gates/:gateId/lock', async (req, res) => {
  ...
  const gateId = Number(req.params.gateId);
  const { rows } = await query(
    `UPDATE gates SET unlocked = FALSE, unlocked_at = NULL, unlocked_by = NULL
      WHERE id = $1 AND organization_id = $2 RETURNING id, unlocked`,
    [gateId, orgId]
  );
  if (!rows[0]) return res.status(404).json({ error: 'Gate not found' });
  res.json({ gate: rows[0] });
