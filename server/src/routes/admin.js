...
router.get('/submissions/:id', async (req, res) => {
  ...
  const modelRes = await query(
    `SELECT field_id, answer_text, commentary, model_data, field_options_override
  ...
  for (const m of modelRes.rows) {
    ...
    if (m.field_options_override) {
      overrideByFieldId.set(m.field_id, m.field_options_override);
    }
  }
